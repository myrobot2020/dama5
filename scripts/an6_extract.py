...
# Sutta/commentary split rules (same as Book 5)
...
def update_sutta_tag_csv(*, repo_root: Path, out_prefix: str, rows_to_add: list[dict[str, str]]) -> Path:
    csv_path = repo_root / "sutta_tag.csv"
    header = [
        "sutta_id",
        "source_path",
        "sutta_word_count",
...
def extract_book(*, repo_root: Path, source_txt_path: Path, volume: int, out_prefix: str) -> Outputs:
    ...
    for sid, start, end in merged_spans:
        ...
        sutta_txt, comm_txt, _used_rule = split_sutta_commentary(block)
    ...
    updated_csv = update_sutta_tag_csv(repo_root=repo_root, out_prefix=out_prefix, rows_to_add=csv_rows)
    return Outputs(suttas_written=written, commentary_missing=missing_commentary, updated_csv=updated_csv)


def main() -> None:
    repo_root = Path(__file__).resolve().parents[1]
    src = repo_root / "raw2" / "AN_Book_06.txt"
    outs = extract_book(repo_root=repo_root, source_txt_path=src, volume=6, out_prefix="an6")
    print(f"Wrote {outs.suttas_written} per-sutta JSON files to an6/suttas.")
    print(f"Commentary missing/empty: {outs.commentary_missing} of {outs.suttas_written}")
    print(f"Updated: {outs.updated_csv}")
